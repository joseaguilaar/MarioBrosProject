...
### A. Example of "MazeMaster" Logic
The following ASCII representation illustrates the "Lock" generated by our system:

```text
      [ ] [ ]  <- Stone Wall (Impassable)
      [ ] [ ]
      [ ] [ ]
[?][?][#][#]   <- Brick Bridge (Breakable from below)
      ^  ^
      |  |
Player must jump here (Requires Big Mario)
